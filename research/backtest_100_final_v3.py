...
def run_100_v3():
    ...
    # Pre-calc All VWMA lengths (20-300)
    vwmas = {}
    pv = close * volume
    for w in range(20, 301):
        vwmas[w] = pv.rolling(window=w).mean() / volume.rolling(window=w).mean()
        
    # Grid Search: Slopes + Years
    # Slopes: [20, 22, 25, 28, 30]
    # Intercept: 140
    # RSI: 85, Stoch: 0.9 (Fixed winning combo)
    
    slopes = [20, 22, 25, 28, 30]
    
    results = []
    
    print(f"Optimization Loop ({len(slopes)} combos)...")
    
    for slope in slopes:
        
        # Calc MA length series
        ...
        current_asset = 'Cash'
        fee = 0.002
        start_idx = 300 # Start 2017ish? 
        # Dates[300] is roughly Late 2017?
        # Dates[0] is 2016-09. 300 days = 2017-09?
        # Let's check dates later.
        ...
        results.append({'Slope': slope, 'CAGR': cagr})
        print(f"  Slope {slope}: {cagr*100:.2f}%")
        
    results.sort(key=lambda x: x['CAGR'], reverse=True)
    print("\n=== FINAL V3 RESULTS ===")
    print(f"Best: Slope {results[0]['Slope']} => {results[0]['CAGR']*100:.2f}%")
