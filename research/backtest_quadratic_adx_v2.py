...
def run_quadratic_adx_v2():
    ...
    # Base NATR (for RSI Period and Crash Guard)
    ...
    # Fine-Tuning Grid Search for ADX
    # Best so far: A=0.5, B=-30, C=160
    
    As = [0.3, 0.4, 0.5, 0.6, 0.7]
    Bs = [-32, -31, -30, -29, -28]
    Cs = [160] # Fixed for now
    
    results = []
    
    print(f"ADX Fine-Tuning Loop ({len(As)*len(Bs)*len(Cs)} combos)...")
    ...
    print("\n=== QUADRATIC ADX V2 RESULTS ===")
    top = results[0]
    print(f"Best: {top['Config']} => {top['CAGR']*100:.2f}%")

if __name__ == "__main__":
    run_quadratic_adx_v2()
