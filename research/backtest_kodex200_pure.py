...
def run_kodex200_pure_comparison():
    print("Fetching KODEX 200 Data...")
    # 069500.KS is KODEX 200
    df = yf.download('069500.KS', start='2010-01-01', progress=False)
    ...
    pv = close * volume
    vwma_cache = {}
    # We'll calculate specific lengths needed in loop to save memory/time
    ...
    fee = 0.001 # lower fee for 1x? no, stick to 0.001 for 1x is fair, or 0.002 to be safe.
    # Let's use 0.001 (0.1%) as 1x is less volatile and tighter spreads.
    ...
    for i in range(start_idx, len(dates)-1):
        ...
        if not pd.isna(c_natr) and c_natr > 5.0:
            ...
            if pd.isna(ma_val): target = 'Cash'
                ...
                target = 'Cash' # NO INVERSE allowed
    ...
    qs.reports.html(
        ...
        output='pure_kodex200_comparison.html',
        title='Strategy (1x KODEX 200 Only) vs Buy & Hold'
    )
